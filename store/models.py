...
class Menu(models.Model):
    ...
    menu_name = models.CharField(
        max_length=255, choices=CHOICES, unique=True, verbose_name='Имя пункта меню')
    show_menu = models.BooleanField(
        default=True, verbose_name='Включить в меню')
...
class Category(models.Model):
    category_name = models.CharField(
        max_length=255, verbose_name='Имя категории')
    menu_item = models.ManyToManyField(
        'Menu', verbose_name='Название элемента меню', related_name='menu_item')
...
class ImageCollection(models.Model):
    image_url = models.ImageField(
        upload_to='products', blank=True, verbose_name='Изображение коллекции')
...
class Collection(models.Model):
    collection_name = models.CharField(
        max_length=255, verbose_name='Название коллекции')
    description = models.TextField(
        blank=True, verbose_name='Описание коллекции')
    images = models.ManyToManyField(
        'ImageCollection', verbose_name='Изображание коллекции')
...
class ProductImage(models.Model):
    image_url = models.ImageField(
        upload_to='products', blank=False, verbose_name='Изображение товара')
...
class Color(models.Model):
    ...
    color_name = models.CharField(
        max_length=255, verbose_name='Название цвета', null=True)
    color_hex = models.CharField(
        choices=COLOR_CHOICES, verbose_name='Цвет', max_length=255)
...
class Size(models.Model):
    ...
    name = models.CharField(
        max_length=255, choices=CHOICES,  verbose_name='Размер')
...
class Product(models.Model):
    collection = models.ForeignKey(
        'Collection', verbose_name='Коллекция', blank=False, on_delete=models.CASCADE, null=True)
    product_name = models.CharField(
        max_length=255, blank=False, verbose_name='Название товара')
    price = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name='Цена', blank=False)
    ...
    model_parameters = models.CharField(
        max_length=255, verbose_name='Параметры модели')
    size_on_the_model = models.CharField(
        max_length=10, verbose_name='Размер на модели', blank=True)
    description = models.TextField(verbose_name='Описание', blank=False)
    colors = models.ManyToManyField(
        Color, through='ProductColor', related_name='products')
    ...
    quantity = models.PositiveIntegerField(
        null=True, verbose_name='Количество товара')
    date = models.DateTimeField(
        verbose_name='Дата создания', auto_now_add=True, null=True)
    related_products = models.ManyToManyField(
        'self', verbose_name='Связанные товары', blank=True)
    views = models.ManyToManyField(
        'ProductView', verbose_name='Просмотры продукта')
...
class ProductColor(models.Model):
    product = models.ForeignKey(
        Product, on_delete=models.CASCADE, related_name='productcolors')
    color = models.ForeignKey(Color, on_delete=models.CASCADE)
    images = models.ManyToManyField(
        'ProductImage', verbose_name='Изображения товара')
...
class Order(models.Model):
    ...
    status = models.CharField(
        max_length=100, choices=CHOICES_ORDER, default='created')
    amount = models.DecimalField(max_digits=10, decimal_places=2)

    # Данные клиента
    first_name = models.CharField(
        max_length=255, verbose_name='Имя', null=True)
    last_name = models.CharField(
        max_length=255, verbose_name='Фамилия', null=True)
    email = models.EmailField(verbose_name='Email', null=True)
    phone = models.CharField(
        max_length=20, verbose_name='Телефон', null=True, blank=True)
    city = models.CharField(
        max_length=255, verbose_name='Населённый пункт', null=True)
    delivery_method = models.CharField(
        max_length=255, verbose_name='Способ доставки', null=True)
    ...
    apartment_office = models.CharField(
        max_length=10, verbose_name='Квартира/офис', null=True)
...
class PaymentRecord(models.Model):
    order = models.OneToOneField(
        'Order', on_delete=models.CASCADE, related_name='payment_record_order')
    amount = models.DecimalField(max_digits=10, decimal_places=2)
